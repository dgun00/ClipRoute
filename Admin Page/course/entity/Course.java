...
public class Course extends BaseEntity {
    ...
    // Admin page
    public static Course createFromAdmin(String title, Integer travelDays, Region region, Video sourceVideo, Member author) {
        Course c = new Course();
        c.title = title;
        c.travelDays = travelDays;
        c.region = region;
        c.sourceVideo = sourceVideo;
        c.author = author;         // 관리자 계정 매핑
        c.isCustomized = false;    // 관리자 원본이므로 false
        c.isRep = true;            // 대표 코스로 설정
        c.deletedAt = null;
        return c;
    }
    ...
    public static Course createScrappedFrom(Course original, Member author) {
        ...
        c.isCustomized = true; // 스크랩 직후는 true임
        c.deletedAt = null;
        return c;
    }
