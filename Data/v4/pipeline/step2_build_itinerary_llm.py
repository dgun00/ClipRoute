...
def _build_candidates_from_description(video_id: str, lines: List[str]) -> List[dict]:
    ...
    for ln in lines:
        ...
        title_clean, day_hint = _strip_day_token(title_part)
        if day_hint > 0:
            current_day = day_hint

        place_name = _guess_place_name_from_chapter_title(title_clean)
        if not place_name:
            continue

        order_by_day.setdefault(current_day, 0)
        order_by_day[current_day] += 1

        candidates.append(
            {
                "video_id": video_id,
                "day": current_day,
                "order": order_by_day[current_day],
                "seconds": sec,
...
def _find_seconds_from_cues(place_name: str, cues: List[Dict[str, Any]]) -> int:
    """Return earliest cue start seconds where cue text contains place_name (case-insensitive).
    ...
    for c in cues:
        try:
            ...
            if not txt:
                continue
            if name_lc in txt.lower():
                start = c.get("start")
                # start is float seconds
                try:
...
def run(materials: List[dict]) -> Dict[str, List[dict]]:
    ...
    for item in materials:
        ...
        # 영상 처리 시작 로그(타이틀 포함)
        log("step2", "video", video_id=video_id, title=title)
        ...
        if chapter_candidates:
            log("step2", "desc_chapters", video_id=video_id, title=title, count=len(chapter_candidates))
        ...
        for i, chunk in enumerate(chunks):
            log("llm", "normalize chunk", video_id=video_id, title=title, chunk=f"{i+1}/{len(chunks)}")
            try:
        ...
        if vtt_cues:
            ...
            log("step2", "vtt_cues", video_id=video_id, title=title, cues=len(vtt_cues), filled=seconds_filled_from_vtt)
        ...
        if dropped_generic:
            log("step2", "drop_generic", video_id=video_id, title=title, dropped=dropped_generic, kept=len(filtered_collected))
        ...
        for day in sorted(by_day.keys()):
            items = by_day[day]
            # seconds가 0인 것은 뒤로; mention_idx 반영
            items.sort(
                key=lambda x: (
        ...
        log("step2", "places_raw", video_id=video_id, title=title, count=len(uniq))
