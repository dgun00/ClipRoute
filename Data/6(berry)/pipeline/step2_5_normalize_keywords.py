...
def run(raw: Dict[str, List[dict]]) -> Dict[str, List[dict]]:
    ...
    _FALLBACK_KEEP_RE = re.compile(r"^[가-힣0-9 ]{2,15}$")

    def _looks_like_sentence_or_meta(s: str) -> bool:
        """Heuristic filter for non-place phrases (sentences/meta words).

        - too long
        - too many spaces
        - contains obvious meta markers like vlog/ep
        - contains emoji or unusual symbols
        """
        t = (s or "").strip()
        if not t:
            return True
        if len(t) > 30:
            return True
        # many spaces usually indicates a sentence
        if t.count(" ") >= 2:
            return True
        low = t.lower()
        # meta markers (minimal but effective)
        if "vlog" in low or "브이로그" in t or low.startswith("ep"):
            return True
        # emoji / decorative symbols
        if re.search(r"[\U0001F300-\U0001FAFF]", t):
            return True
        # excessive punctuation/special chars (keep basic separators only)
        if re.search(r"[^가-힣0-9A-Za-z \-]", t):
            # allow asterisk and slash to appear in titles but treat as meta
            return True
        return False

    def _allow_fallback_name(phrase: str) -> bool:
        """Allow using the original phrase as search_name only when it's very likely a place token."""
        p = (phrase or "").strip()
        if not p:
            return False
        if _looks_like_sentence_or_meta(p):
            return False
        # allow at most one space (e.g., '성산 일출봉')
        if p.count(" ") > 1:
            return False
        return bool(_FALLBACK_KEEP_RE.match(p))

    def _is_must_keep(p: dict) -> bool:
        """Preserve strong candidates from title/description seeds, but never preserve obvious meta/sentences."""
        if not isinstance(p, dict):
            return False
        phrase = (p.get("place_name") or "").strip()
        if _looks_like_sentence_or_meta(phrase):
            return False
    ...
    for idx, (video_id, places) in enumerate(raw.items(), 1):
        ...
        for p in places:
            ...
            if not n:
                # If LLM didn't return a mapping, only keep when the phrase is safe to fallback.
                if must_keep and _allow_fallback_name(phrase):
                    merged.append(
                        {
                            **p,
                            "search_name": phrase,
            ...
            if n.get("candidate_drop"):
                # If LLM says drop, do NOT revive by falling back to the raw phrase.
                # Only allow override when LLM still provides a usable search_name.
                if must_keep:
                    sn = (n.get("search_name") or "").strip()
                    if sn:
                        merged.append(
                            {
                                **p,
                                ...
                                "keyword_reason": "must_keep_override_candidate_drop_with_search_name",
            ...
            if not search_name:
                # Only allow fallback when phrase itself looks like a place token.
                if must_keep and _allow_fallback_name(phrase):
                    merged.append(
                        {
                            **p,
