...
def _find_seconds_from_cues(place_name: str, cues: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Find earliest cue where cue text contains place_name (case-insensitive).
...
def run(materials: List[dict]) -> Dict[str, List[dict]]:
    ...
    for video_idx, item in enumerate(materials, 1):
        ...
        for p in collected:
            ...
            # ✅ must_keep candidates survive even if generic (mark only)
            if p.get("must_keep"):
        ...
        for day in sorted(by_day.keys()):
            ...
            for cand in items:
                ...
                for j, kept in enumerate(compact):
                    if (kept.get("place_name") or "").strip() != pn:
                        continue
