...
//    private final CourseDetailConverter courseDetailConverter;
    ...
    // [2번 API]
    @Override
    public CourseResDTO.CoursePublicListDTO getCourseList(Long regionId, Integer travelDays, String sort, Long lastCourseId, Integer lastValue, Integer pageSize) {

        // No-Offset 방식이므로 page는 항상 0으로 고정하고 size만큼 가져옴
        Pageable pageable = PageRequest.of(0, pageSize);

        // 리포지토리 호출하여 슬라이스 데이터 조회
        Slice<Course> courseSlice = courseRepository.findCoursesByFilter(regionId, travelDays, sort, lastCourseId, lastValue, pageable);


        // 첫 페이지 진입 시(lastCourseId == null)에만 전체 개수 조회
        Long totalElements = (lastCourseId == null)
                ? courseRepository.countCoursesByFilter(regionId, travelDays)
                : null;

        // 엔티티 리스트를 DTO 리스트로 변환 (2번 API는 추천 로직이 없으므로 isRecommended는 항상 false)
        ...
        // 최종 응답 DTO 조립 및 반환
        return CourseConverter.toCoursePublicListDTO(coursePublicDTOs, courseSliceDTOs, totalElements, sort);
    }

    // [3번 API]
    @Override
    public CourseResDTO.CoursePublicListDTO getCourseListWithRecommendation(Long regionId, Integer travelDays, String sort, int page, int size) {

        Pageable pageable = PageRequest.of(page, size);

        // 리포지토리 호출
        Slice<Course> courseSlice = courseRepository.findCoursesByFilterWithRecommendation(regionId, travelDays, sort, pageable);

        // 정확히 일치하는 데이터 개수 조회 (첫 페이지일 때만)
        ...
        List<CourseResDTO.CoursePublicDTO> coursePublicDTOs = courseSlice.stream()
                .map(course -> {
                    // 사용자의 입력 조건(regionId, travelDays)과
    ...
    // 스크랩,여행상태 record 타입 , 비로그인시 디폴트값 넣는 메서드
